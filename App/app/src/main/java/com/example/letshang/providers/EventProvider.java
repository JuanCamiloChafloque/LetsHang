...
public class EventProvider {
     ...
     * @return
     */
    public List<Event> getFilteredEvents(int maxDistance, LatLng currentLocation, boolean isAcademic, boolean isMusic,
                                         boolean isGame, boolean isSocial, boolean isSports){

        List<Event> ans = new ArrayList<>();

        for(int i = 0 ; i < eventsList.size() ; ++i){
            if(filter(eventsList.get(i), maxDistance , currentLocation, isAcademic, isMusic, isGame, isSocial, isSports)){
    ...
    public boolean filter(Event e , int maxDistance, LatLng currentLocation, boolean isAcademic, boolean isMusic,
                          boolean isGame, boolean isSocial, boolean isSports){

        //Check distance from current location to the location from events
        if(distanceBetween(currentLocation, e.getLocation()) > maxDistance){
            return false;
        }

        //Check type of event
        if(e instanceof AcademicEvent && !isAcademic){
            return false;
        } else if (e instanceof GameEvent && !isGame){
            return false;
        } else if (e instanceof  MusicEvent && !isMusic){
            return false;
        } else if (e instanceof SportEvent && !isSports){
            return false;
        } else if (e instanceof SocialEvent && !isSocial) {
            return false;
        }
        //Check dates
